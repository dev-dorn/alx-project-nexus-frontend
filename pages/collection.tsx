...
const Collection: React.FC = () => {
  ...
  const applyFilter = () => {
    ...
    if (category.length > 0) {
      productsCopy = productsCopy.filter((item) => category.includes((item.category || '') as string));
    }
    if (subCategory.length > 0) {
      productsCopy = productsCopy.filter((item) => subCategory.includes((item.subCategory || '') as string));
    }
    ...
    <div className="flex flex-col sm:flex-row gap-1 sm:gap-10 pt-10 border-t">
      <div className="min-w-60">
        <p onClick={() => setShowFilter(!showFilter)} className="my-2 text-xl flex items-center cursor-pointer gap-2">
          Filters
          <img className={`h-3 sm:hidden ${showFilter ? "rotate-90" : ""}`} src={assets.dropdown_icon} alt="filters" />
        </p>
